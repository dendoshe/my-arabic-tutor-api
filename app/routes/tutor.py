...
async def process_translation(
    ...
    if include_english:
        raw_arabic, english_text = translator_func(text)
    else:
        raw_arabic = translator_func(text)
        english_text = None
    ...
    if include_english:
        return english_text, improved_arabic, translit_text
    else:
...
async def translate_sv(req: TranslateRequest):
    english, arabic_script, arabic_translit = await process_translation(
...
async def translate_en(req: TranslateRequest):
    arabic_script, arabic_translit = await process_translation(
        req.text, translate_en_to_ar, include_english=False
    )
